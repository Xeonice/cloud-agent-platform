/**
 * Detached terminal-session command contract. Kept as a dependency-free leaf
 * under the provider-neutral sandbox center so every provider transport uses the
 * same tmux session names, attach/liveness probes, resize commands, and headless
 * exit sentinel.
 */
...
/** Exact tmux target syntax prevents prefix matches when task ids share a stem. */
export function exactDetachedSessionTarget(taskId: string): string {
...
export function buildAttachSessionCommand(taskId: string): string {
  const target = exactDetachedSessionTarget(taskId);
  return (
    `${TMUX_UTF8} set-window-option -t ${target}: window-size manual \\; ` +
...
/**
 * Attach a disposable browser viewer to the exact task session on tmux's
 * default socket. `window-size manual` establishes the invariant that a newly
 * attached client cannot become tmux's implicit size authority: tmux 3.2/3.3
 * may still resize a zero-client `latest` window even when attach receives
 * `ignore-size`. The flag remains as a second attachment-local guard. The
 * gateway's current writer applies authoritative geometry separately.
 */
export function buildViewerAttachSessionCommand(taskId: string): string {
  const target = exactDetachedSessionTarget(taskId);
  return (
    // tmux 3.3a parses set-option's target as target-pane. The trailing colon
    // disambiguates the exact session; without it `=task...` is treated as a
    // pane/window token and fails even though attach-session accepts it.
    `${TMUX_UTF8} set-window-option -t ${target}: window-size manual \\; ` +
    `set-option -t ${target}: status off \\; ` +
    `attach-session -f ignore-size -t ${target}`
  );
...
export function buildExactHasSessionCommand(taskId: string): string {
  return `${TMUX_UTF8} has-session -t ${exactDetachedSessionTarget(taskId)}`;
}
...
export function buildResizeDetachedSessionCommand(
  ...
  return `${TMUX_UTF8} resize-window -t ${exactDetachedSessionTarget(taskId)}: -x ${cols} -y ${rows}`;
}
