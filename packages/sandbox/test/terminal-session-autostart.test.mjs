/**
 * Focused integration test for the shared terminal session engine under the
 ...
 * fast. Imports the built sandbox center and composes the shared engine with
 * AIO provider adapters against a fake sandbox.
 */
...
function startFakeSandbox(opts = {}) {
  ...
  const http = createServer((req, res) => {
    ...
    req.on('end', () => {
      const url = req.url || '';
      if (url === '/v1/shell/sessions/create' && req.method === 'POST') {
        let id = '';
        try {
        ...
        shellSessions.add(id);
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end(
          JSON.stringify({
            success: true,
            data: { session_id: id, working_dir: '/home/gem' },
          }),
        );
        return;
      }
      if (url.startsWith('/v1/shell/sessions/') && req.method === 'DELETE') {
        const id = decodeURIComponent(url.slice('/v1/shell/sessions/'.length));
        shellSessions.delete(id);
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end(JSON.stringify({ success: true, data: { session_id: id } }));
        return;
      }
      if (url.startsWith('/v1/shell/exec')) {
        ...
        if (!shellSessions.has(id)) {
          ...
          res.end(JSON.stringify({ success: false, message: `Session ${id} not found` }));
          return;
        }
    ...
    wss.on('connection', (ws) => {
      ...
      ws.on('message', (raw) => {
        ...
        if (
          ...
          /(?:^|\s)tmux -u new-session -d -s task/u.test(frame.data)
        ) {
          // The fake liveness endpoint must model the same state transition as
          // real tmux: once the accepted launch input creates a detached task
          // session, the next `has-session` probe observes it alive. Otherwise
          // the 30 ms test poller reports a fictitious unobserved exit before
          // Codex can emit its startup DSR.
          sessionAlive = true;
        }
      });
...
function withConfirmedFakeProviderCleanup(factory) {
  return {
    open() {
      const transport = factory.open();
      // This in-process server models the session engine, not AIO's two-PTY
      // ownership protocol. Give the fake provider an explicit exact-cleanup
      // settlement so owner-recovery tests do not accidentally exercise the
      // legacy one-socket transport's intentionally indeterminate cleanup.
      return {
        get readyState() {
          return transport.readyState;
        },
        get opaqueInputCapability() {
          return transport.opaqueInputCapability;
        },
        cleanupDecision: Promise.resolve({
          kind: 'confirmed',
        ...
        sendInputBytes: (data) => transport.sendInputBytes(data),
        sendTerminalResponseBytes: (data) =>
          transport.sendTerminalResponseBytes(data),
        sendResize: (cols, rows) => transport.sendResize(cols, rows),
        sendPong: (timestamp) => transport.sendPong(timestamp),
...
async function main() {
  ...
  class TestSandboxTerminalSession extends sandboxModule.SandboxTerminalSession {
    constructor(
        ...
        mode === 'launch-or-attach'
          ? async () => ({
              ...
              runtime: {
                ...
                buildLaunchLine: (ctx) =>
                  sandboxModule.buildDetachedCodexLaunchLine(
                    ctx.taskId,
                    sandboxModule.DEFAULT_CODEX_INTERACTIVE_LAUNCH_ARGV,
                  ),
      ...
      super(
        ...
        commandExecutor ??
          sandboxModule.createAioHttpCommandExecutor({ baseUrl }),
        async (intent) => intent,
        undefined,
        undefined,
        undefined,
  ...
  const AioPtyClient = TestSandboxTerminalSession;

  // --- Case 0: provider story fixture uses the same detached task session as
  //     production so disposable viewers can attach to it. --------------------
  {
    ...
    const launchDecision = await client.launchDecision;
    assert(launchDecision.kind === 'launched', 'provider story launch settles explicitly');
    assert(execCalls.length === 2, 'provider story installs then launches one detached session');
    const heredocStart = "<<'CAP_PROVIDER_TERMINAL_STORY_SCRIPT'\n";
    const scriptStart = execCalls[0].command.indexOf(heredocStart) + heredocStart.length;
    const scriptEnd = execCalls[0].command.lastIndexOf('\nCAP_PROVIDER_TERMINAL_STORY_SCRIPT');
    const fixtureScript = execCalls[0].command.slice(scriptStart, scriptEnd);
    ...
    assert(
      scriptStart >= heredocStart.length && scriptEnd > scriptStart && syntax.status === 0,
      `installed provider story Node fixture is syntactically valid${
        syntax.status === 0 ? '' : `: ${String(syntax.stderr).trim()}`
      }`,
    );
    assert(/PROVIDER_STORY_BEGIN/.test(execCalls[0].command), 'fixture script emits begin marker');
    assert(
      /PROVIDER_STORY_CURRENT_FRAME/.test(execCalls[0].command) &&
        execCalls[0].command.includes('write(ESC + "[?1049h"'),
      'fixture script renders a deterministic native alternate-screen frame',
    );
    assert(/PROVIDER_STORY_ECHO/.test(execCalls[0].command), 'fixture script echoes input');
    assert(
      /PROVIDER_STORY_ORACLE_UTF8/.test(execCalls[0].command) &&
        /PROVIDER_STORY_ORACLE_MOUSE/.test(execCalls[0].command) &&
        /setRawMode\(true\)/.test(execCalls[0].command),
      'fixture script records raw UTF-8 and legacy mouse bytes at the provider pane',
    );
    assert(
      /CAP_STORY_FREEZE/.test(execCalls[0].command) &&
        /PROVIDER_STORY_LIVE_PROBE exactly-once/.test(execCalls[0].command),
      'fixture script supports a deterministic freeze and one-shot live delta',
    );
    assert(
      /tmux -u new-session -d -s taskterminal-story-test/.test(execCalls[1].command) &&
        /node \/tmp\/cap-provider-terminal-story\.mjs/.test(execCalls[1].command),
      'fixture mode starts the script in the exact detached task session',
    );
    assert(
      /attach-session -f ignore-size -t =taskterminal-story-test/.test(
        transport.input.join('\n'),
      ),
      'owner attaches to the same detached task session',
    );
    assert(
  ...
  //     then auto-submit. ------------------------------------------------------
  {
    ...
    if (launch) {
      ...
      assert(
        !launch.includes('--no-alt-screen'),
        'launch line leaves Codex in its native terminal mode',
      );
    }
    assert(
