 ...
 * provider-neutral terminal session engine.
 *
 ...
 *     codex never starts).
 *
 * This drives the real shared session engine with the AIO transport adapter
 * against a fake AIO terminal WS server:
...
async function main() {
  ...
  const createCodexClient = (taskId, sandbox) =>
    new SandboxTerminalSession(
      ...
      createAioTerminalTransportFactory({ taskId, wsUrl: sandbox.wsUrl }),
      {
        async exec(request) {
          return {
            exitCode: 0,
