...
/**
 * The EXACT `zod` instance every `@cap-console/contracts` schema is built on.
 *
 * Why this is re-exported (public-v1-api, Integration 4.1):
 *
 * `@cap-console/contracts` is an ESM package (`"type": "module"` → its built schemas
 * `import { z } from 'zod'`, resolving zod's ESM entry `index.js`). A CJS consumer
 * (e.g. `@cap-console/api`, `"type": "commonjs"`) that does `require('zod')` resolves zod's
 ...
 * "extend the shared `@cap-console/contracts` z instance" literally true rather than
