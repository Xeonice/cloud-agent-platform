...
await test('REST client covers missing, failed, and invalid edge responses', async () => {
  ...
  const execShape = new mod.BoxLiteRestClient({
    ...
    fetch: makeFetch({
      ...
      'GET /v1/sandboxes/camel-rootfs': response(200, {
        id: 'camel-rootfs',
        rootfsPath: '/rootfs/camel.img',
      }),
    }),
  });
  ...
  assert.equal((await execShape.getSandbox('camel-rootfs')).rootfsPath, '/rootfs/camel.img');
...
await test('config parser covers defaults, require helper, and invalid modes', () => {
  ...
  assert.throws(
    () =>
      mod.resolveBoxLiteSandboxSource({
        config: {
          defaultImage: 'img',
          imageByRuntime: {},
          defaultRootfsPath: '/rootfs/default.img',
          rootfsPathByRuntime: {},
        },
        runtimeId: null,
      }),
    /resolves to both image and rootfs path/,
  );
  assert.throws(
    () =>
      mod.resolveBoxLiteSandboxSource({
  ...
  assert.throws(
    ...
    /default has no image or rootfs path/,
  );
  ...
  assert.equal(
    mod.readBoxLiteProviderConfig(validEnv({
      BOXLITE_IMAGE: '',
      BOXLITE_ROOTFS_PATH_MAP: 'bad=entry=shape',
    })).status,
    'invalid',
  );
  assert.equal(
    mod.readBoxLiteProviderConfig(validEnv({
      BOXLITE_IMAGE_MAP: 'codex=img',
      BOXLITE_ROOTFS_PATH_MAP: 'codex=/rootfs/codex.img',
    })).status,
    'invalid',
  );
...
await test('provider covers fallback URLs, stale sandboxes, local descriptors, and archive guards', async () => {
  ...
  const existingRootfsClient = new mod.FakeBoxLiteClient();
  existingRootfsClient.sandboxes.set('cap-boxlite-existing-rootfs', {
    id: 'cap-boxlite-existing-rootfs',
    taskId: 'existing-rootfs',
    state: 'running',
    rootfsPath: '/rootfs/existing.img',
  });
  const existingRootfsProvider = new mod.BoxLiteSandboxProvider({
    config: validConfig({
      BOXLITE_IMAGE: '',
      BOXLITE_ROOTFS_PATH: '/rootfs/default.img',
    }),
    client: existingRootfsClient,
  });
  const existingRootfsRun = await existingRootfsProvider.getSelectedSandboxRun('existing-rootfs');
  assert.equal(existingRootfsRun.preflight.image, '/rootfs/existing.img');
