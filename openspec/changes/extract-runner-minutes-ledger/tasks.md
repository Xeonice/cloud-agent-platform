...
- [x] 6.1 Remove `apps/api/src/guardrails/ground-truth-publishing-failure-does-not-disturb-transition.spec.ts` from the tree (or relocate it outside `apps/api/src/guardrails/` if its coverage is judged worth keeping permanently). The file is a 229-line runnable `*.spec.ts` added during verification as a ground-truth probe for the MODIFIED `guardrails` scenario "Publishing failure does not disturb the transition"; its own header declares it "not part of the permanent suite". It is currently **untracked but present**, and it names `extract-runner-minutes-ledger` in its header, so it is this change's working-set artifact. While it sits there the guardrails characterization baseline this change is required to leave untouched is **7 `*.spec.ts` files / 136 `test()`** instead of the pinned **6 files / 135 `test()`** (`ls apps/api/src/guardrails/*.spec.ts | wc -l` → 7; `grep -ho 'test(' apps/api/src/guardrails/*.spec.ts | wc -l` → 136; excluding this one file both numbers return to 6 / 135 exactly). It also makes the change's own `assertion-rewrite-ledger.md` §3 claim ("Guardrails-directory spec or `.test.mjs` files edited: **0** — the guardrails characterization baseline … is untouched by this change") false on the live tree, and it would enter the diff the moment archive stages the working tree. Acceptance: `ls apps/api/src/guardrails/*.spec.ts | wc -l` = 6, `grep -ho 'test(' apps/api/src/guardrails/*.spec.ts | wc -l` = 135, `ls apps/api/src/guardrails/*.test.mjs | wc -l` = 8, `git status --porcelain apps/api/src/guardrails/` empty, and `assertion-rewrite-ledger.md` §3 re-reads true without edit. Do **not** satisfy this by weakening the baseline numbers in either spec file — the frozen baseline is the requirement, not a reporting convenience.
  ...
  - surfaces: ["developer-workflow"]
  - verify: "workflow-gates"
- [x] 6.2 Repair the public-surface gate this branch broke, so the requirement's fourth scenario ("the public-surface position is executed, not assumed") can actually be re-proven by running the verifier. **The failure is live and reproducible on HEAD**, not a stale transcript: `CAP_PUBLIC_SURFACE_BASE_SHA=$(git rev-parse main) node scripts/public-surface-adversarial.mjs verify extract-runner-minutes-ledger` now returns `"passed": false` with `command.exitCode: 1` and five findings — one `public-surface-gate-failed` plus four `dynamic-evidence-missing` (lanes `registry`, `restMetadata`, `mcpSdkMetadata`, `behavior`). Root cause is **this branch's own commit `ee0dc70`** ("fix(workflows): stop opsx-verify from failing a change with its own probes"), which added two new `agent()` calls to `.claude/workflows/opsx-verify.js` — `probe-hygiene:snapshot` (`:289`) and `probe-hygiene:sweep` (`:382`) — without updating the fake-agent label allowlist in `scripts/public-surface-adversarial.test.mjs`. That helper (`runDeterministicVerdictThroughVerify`) compiles and executes the real workflow source (`compileVerifyWorkflow`, `:167-178`) and `throw new Error('Unexpected workflow agent label: ' + options.label)` at `:278` for any label it does not recognise, so the two new calls throw before the workflow reaches its assertions. `node --test scripts/public-surface-adversarial.test.mjs` is **7 fail / 12 pass**, and the seven are exactly the protocol-regression cases that guard the sidecar contract: "a passing deterministic verdict passes workflow without an LLM dynamic override", "workflow accepts and blocks an evaluator finding after a zero focused-gate exit", "undeclared public impact becomes a blocking specification defect", "a false MCP exclusion becomes a blocking specification defect", "MCP field stripping becomes unmet even when schema and type checks still pass", "an extra REST secret forwarded outside the registry exact set becomes unmet", "an extra MCP internalFlag forwarded outside the registry exact set becomes unmet". `packages/contracts/package.json:25` globs `../../scripts/public-surface-*.test.mjs` into its `test:public-surface` script, which is why the repo-level `pnpm test:public-surface` (`scripts/public-surface-tests.mjs fast`) exits 1 and why all four mandatory dynamic evidence lanes come back unproven. Note this is **not** evidence that a public surface moved — `git diff main...HEAD -- packages/contracts/` is still empty and the four surface statuses in `surface-impact.json` stay `unchanged` with `protocolDifferences` empty; the defect is that the gate which would *prove* that claim cannot currently run, so the claim is unproven rather than false. **Fix the allowlist, not the workflow**: teach `runDeterministicVerdictThroughVerify` to answer the `probe-hygiene:snapshot` / `probe-hygiene:sweep` labels with a valid `PATHS`-shaped result (`{ paths: [] }` is the honest fixture value — the fixture tree has no probes) rather than deleting the hygiene calls from `.claude/workflows/opsx-verify.js`, and prefer a shared label→stub map over a seventh copy of the same throw so the next workflow-side `agent()` call cannot silently blind the same seven tests again. Do **not** satisfy this by loosening the throw into a permissive default — the throw is what makes an unmodelled workflow step visible. Acceptance: `node --test scripts/public-surface-adversarial.test.mjs` → 19 pass / 0 fail; `pnpm test:public-surface` → exit 0; `CAP_PUBLIC_SURFACE_BASE_SHA=$(git rev-parse main) node scripts/public-surface-adversarial.mjs verify extract-runner-minutes-ledger` → `"passed": true`, `command.exitCode: 0`, `sidecar`/`registry`/`restMetadata`/`mcpSdkMetadata`/`behavior` all `passed: true`, `findings: []`; and `gate-results.md` task 5.12's recorded transcript is re-run and re-recorded against the current HEAD rather than left at its `cce2b2d`-era text.
  - requirements: ["runner-minutes-accounting/the-ownership-move-adds-no-runtime-behavior-and-no-observable-output-change"]
  - surfaces: ["public-v1", "mcp", "openapi", "playground"]
  - verify: "workflow-gates"
