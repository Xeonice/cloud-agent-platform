...
- [x] 10.8 Move AIO terminal transport and provider-wire lifecycle out of `apps/api/src/terminal` into the AIO provider terminal harness; compose it with the provider-neutral session engine in `@cap/sandbox`.
- [x] 10.9 Move BoxLite terminal transport/client wire lifecycle out of `apps/api/src/terminal` into the BoxLite provider terminal harness; compose it with the same provider-neutral session engine.
