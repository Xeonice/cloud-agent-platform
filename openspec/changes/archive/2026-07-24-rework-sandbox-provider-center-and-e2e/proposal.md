...
- Move the full AIO backend orchestration out of `apps/api` and into `@cap/sandbox-provider-aio`, including Docker lifecycle, readiness, runtime setup hooks, workspace materialization, terminal and command descriptors, retention, provider transport lifecycle, command executors, and readoption; keep the provider-neutral agent terminal session engine in `@cap/sandbox`.
