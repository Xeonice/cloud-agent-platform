...
`@cap/sandbox` owns the provider-neutral terminal session engine: launch-or-attach decisions, detached-tmux commands, initial-ready sequencing, liveness supervision, runtime-declared DSR/CPR startup handling, resize coordination, and normalized exit resolution. AIO owns only AIO JSON/WebSocket translation, its HTTP command/wait protocol, and provider connection mechanics. BoxLite owns REST exec/attach, binary channel translation, resize/control frames, token/header handling, and BoxLite command execution. `@cap/sandbox` owns the registry that resolves those transports from selected-run descriptors and composes either transport with the same session engine.
