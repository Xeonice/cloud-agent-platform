...
#### Scenario: Provider-specific wire mechanics stay with the provider
- **WHEN** AIO or BoxLite needs provider-specific handshake parsing, WebSocket/REST attachment, command/wait protocol handling, binary decoding, resize transport, or credential headers
- **THEN** those wire mechanics are implemented by the owning provider package behind a normalized transport factory
- **AND** the sandbox-center session engine owns shared launch/attach, tmux liveness, runtime startup, reconnect, and exit orchestration while API terminal code receives only normalized terminal events
