# frontend-console Spec Delta

This delta rebuilds the web console from the current Next.js `apps/web` to a TanStack Start (Vite-native, full-stack) application deployed to Vercel via the Nitro `vercel` preset. It introduces the 10-page route tree from the design prototype, a shadcn/ui + Tailwind v4 visual layer with ported design tokens, a shared app-shell, and a TanStack Query data layer with a single real/mock capabilities switch. Backend-supplied data (auth, metrics, history, settings, GitHub import, branch/strategy read-back) is specified in the sibling capabilities (`multi-user-oauth`, `resource-metrics`, `audit-history`, `account-settings`, `github-repository-import`, `repo-and-task-management`); this delta covers only the console (presentation + client data wiring + routing).
