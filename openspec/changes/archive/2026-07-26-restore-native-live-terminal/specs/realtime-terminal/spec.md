...
**Reason**: Replaying a headless SerializeAddon snapshot plus raw `session.log` tail
reconstructs CAP's approximation of a terminal and is not a faithful continuation of a
native alternate-screen TUI. It also couples browser reconnect to durable offsets,
recording order, hidden terminal modes, and viewport repair.

**Migration**: Replace `fromSeq`-based live reconnect with a fresh provider PTY and an
attach-only tmux client. Reset the browser xterm, feed the complete attach redraw, and
then stream new bytes. Keep structured transcripts and bounded owner failure evidence;
leave full raw log/cast disabled by default and never feed an opt-in artifact into a
running terminal reconnect.

### Requirement: The live terminal preserves a scrollable history

**Reason**: Forcing Codex and Claude Code into an inline normal buffer and stripping
tmux alternate-screen sequences changes their native full-screen behavior. Native TUI
fidelity is now the higher-priority requirement, and tmux does not promise that a new
outer terminal client inherits browser scrollback history.

**Migration**: Remove interactive `--no-alt-screen`,
`CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN=1`, live alternate-screen stripping, and viewport
scrollback repair. Running and complete finished raw terminal history are deferred; use
the structured transcript for historical review until a separate bounded history design
is approved.

### Requirement: Reconnect replay remains aligned after API readoption

**Reason**: API readoption no longer rebuilds a browser terminal from a durable
`session.log` byte offset. The load-bearing recovery invariant is the surviving named
tmux session plus an attach-only task owner; browser state is restored independently by
a fresh tmux client redraw.

**Migration**: Keep provider liveness attestation, attach-only owner recovery,
absent/indeterminate outcomes, lifecycle fencing, and nonrecordable owner bootstrap.
After readoption, create viewer PTYs lazily per browser and do not initialize or rebase
live snapshot/tail offsets.
