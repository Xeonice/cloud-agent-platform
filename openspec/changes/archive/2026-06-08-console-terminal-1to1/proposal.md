...
- **Sync the terminal size to the sandbox PTY on connect (NEW — live-verified bug).** Live diagnosis on a running task found codex's PTY stuck at the AIO default **80×24** while the browser xterm auto-fits to **~137×24**, so codex renders in the left 80 columns and its cursor-addressed full-screen redraws + scrollback **history misalign** in the wider browser grid (this is the user-reported "tmux 历史错位"). Root cause: the only path that resizes the PTY is the `resize` frame → `gateway.onResize` (which correctly does `pty.resize` + `resizeHeadless`), but the initial xterm `onResize` fires at mount and RACES the socket open — `sendFrame` drops it when the socket is not yet OPEN — and `onOpen` sends only a `reconnect` frame, whose geometry the backend never applies to the PTY. Fix: the client SHALL send its geometry once the socket is OPEN (`onOpen → sendResize`), and the backend `onReconnect` SHALL resize the PTY + headless from the reconnect frame's cols/rows it already receives. (AIO `/v1/shell/ws` honors `{type:resize}` — live-proven: a probe session sent 137×40 and its PTY became 137×40 — so the gap is purely cap-side wiring.) Independent of the box-removal gate; safe to ship now.
...
- `realtime-terminal`: a NEW requirement **Terminal geometry synced to the sandbox PTY on connect** — the client sends its geometry once the socket is OPEN and the backend `onReconnect` resizes the PTY + snapshot headless from the reconnect frame's cols/rows, so codex renders at the browser size instead of the sandbox default 80×24 (makes the existing "Live-frame parity under PTY parity conditions" precondition actually reachable, and fixes the history misalignment).

## Impact

- **Code (1:1 input, frontend):** `apps/web/src/components/session/session-terminal.tsx` (remove `TerminalCommandInput` render + `sendCommand` + the 150ms `setTimeout`; focus xterm in `onReady`; connection-state affordance). The `TerminalCommandInput` component may remain for the fallback view.
- **Code (size-sync, NEW):** `apps/web/src/components/session/session-terminal.tsx` (`onOpen → sendResize(geo)`) + `apps/api/src/terminal/terminal.gateway.ts` (`onReconnect` resizes `session.pty` + `session.snapshots.resizeHeadless` from the reconnect frame's cols/rows). `gateway.onResize` + `AioPtyClient.resize` + `SnapshotManager.resizeHeadless` already exist and are correct — the fix only ensures the geometry is delivered.
- **Byte passthrough unchanged:** `aio-sandbox-execution` forwards operator keystrokes as `{type:"input"}`; `write-lock-and-takeover` keystroke lease-gating is unchanged (the `onData` path already does takeover-on-first-keystroke).
...
- **Specs:** `openspec/specs/frontend-console/spec.md` (MODIFIED delta — 1:1 input) + `openspec/specs/realtime-terminal/spec.md` (ADDED delta — terminal geometry synced on connect).
