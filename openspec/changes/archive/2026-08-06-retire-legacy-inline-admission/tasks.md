...
## 9. Track: retire-pending-admission-sweep (depends: none)

- [x] 9.1 Retire `recoverPendingAdmissions`, its `runRecoverySafely` wrapper, and both call sites (bootstrap disabled branch + pre-tick step), plus the orphaned `taskBodyFromRow` its deletion left behind — 82 + 25 lines. Found by CI, then by tracing rather than by verify: the sweep filters `task.admissionWork: { is: null }` (`scheduled-tasks.service.ts:1183` on `main`), and durable acceptance writes an admission-work row inside the acceptance transaction (`tasks.service.ts:1317`), so its population is empty by construction — the same emptiness as the boot re-offer, one layer down. ⚠ It also **falsifies a requirement this same change wrote hours earlier**: `Boot recovery re-offers nothing…` called it 'the surviving claim query' and built a disjointness argument on it. Corrected in place in BOTH the live spec and this change's delta, with the error recorded rather than edited away. Not kept as a fail-closed safety net: measured, it cannot progress — recovery re-calls post-commit dispatch, finds no work again, returns `fail-closed` again. A sweep that cannot make progress reads like a recovery path to the next reader, which is worse than none.
  - requirements: ["repo-and-task-management/the-pending-admission-sweep-is-retired-with-the-population-it-served"]
  - surfaces: ["developer-workflow"]
  - verify: "spec-assertions"

- [x] 9.2 Ledger **(c) subject retired** — 8 tests whose subject was the sweep. `scheduled-tasks.service.spec.ts` (7): `post-commit admission failure leaves one pending task for startup recovery`, `startup recovery admits created pending scheduled tasks once`, `competing recovery workers admit one pending scheduled task once`, `recovery drains more than one batch of pending scheduled tasks`, `recovery retains its lease when admission resolves but leaves the task pending`, `recovery outcome fence releases a raced durable work item without spinning`, `run-level admission leases do not block sibling recovery or the next cadence`. `scheduled-tasks-e2e.mjs` (1): `restart recovers a committed pending task and immediately scans overdue schedules` (166 lines). Warrant: the retirement requirement above — the population these drove no longer exists. Test-count accounting, because this repo treats a shrinking suite as evidence of silent coverage loss (apps/api/CLAUDE.md): default api suite **1633 → 1626 = −7**, exactly the seven from the compiled spec; the eighth is in `test/scheduled-tasks-e2e.mjs`, which is `.mjs` not `*.test.mjs` so it runs in its own CI step, not the default suite. Nothing else moved. 1621 pass / 1 fail, the one failure being the pre-existing real-tmux case.
  - requirements: ["repo-and-task-management/the-pending-admission-sweep-is-retired-with-the-population-it-served"]
  - surfaces: ["developer-workflow"]
  - verify: "spec-assertions"

- [x] 9.3 Ledger **(a) re-expressed** — 4 tests whose subject survived the sweep. `application bootstrap keeps polling when recovery fails` → `…when a tick fails`: the invariant (one throw must not stop the poller) is unchanged; only the failure source moved from the retired pre-tick sweep to the tick itself. `application bootstrap does not overlap poll cycles in one process`: the blocking hook moved from the sweep to `tick`, and the assertions moved with it — `recoveryCalls` counters became `tickCalls`, and the mid-flight assertion is now `tickCalls === 1` (many 5ms intervals elapsed, exactly one cycle in flight), which is a sharper statement of the same `pollInFlight` guard. `dispatch retains only the run admission lease when admission leaves the task pending`: dropped the trailing `recoverPendingAdmissions() === 0`; what it proved is structural now — no sweep exists to try. `durable dispatch wakes once, releases the schedule lease, and is excluded from legacy recovery` → `durable dispatch wakes once and releases the schedule lease`: the exclusion half is not a property to check any more, since acceptance always writes the row the sweep used to skip. Plus the cross-surface integration test's recovery sub-scenario (`task-model-cross-surface-e2e.mjs`): the sweep call is gone, `runAfterRecovery` renamed to `dispatchedRun` so the name stops describing a step that no longer runs, and the assertion that dispatch RETAINS the lease is corrected — a durable dispatch releases it, because the durable worker owns the task from there.
  - requirements: ["repo-and-task-management/the-pending-admission-sweep-is-retired-with-the-population-it-served"]
  - surfaces: ["developer-workflow"]
  - verify: "spec-assertions"
