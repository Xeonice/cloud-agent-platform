...
published tag. The existing from-source paths (`make up`, `install.sh` → `make up`) SHALL
remain unchanged and SHALL NOT be a prerequisite of this path.
...
and (when the web profile is enabled) web URLs and the teardown command. If `/health` does not
