...
- [x] 5.2 Live (POST-DEPLOY) — ALL PASS. Round 1 (commit `4ed76a9`, gate fix): ✅ **direct-load gate** — logged out, `/dashboard` + `/tasks/X` → server-side **307 → `/login?redirect=…`** (both SSR + ssr:false; NOT a broken shell / 401 page); ✅ **deep-link return** — login from `/login?redirect=/tasks/deeplink-return-check` landed exactly on `/tasks/deeplink-return-check`; ✅ anon `/` shows GitHub login + visual items (footer, chips, headline breaks, single primary CTA). Round 1 surfaced the **cookie-shadow defect** (a stale host-only `cap_session` 401'd every logged-in browser→api call) → fixed in task 1.6; root cause confirmed by live curl reproduction. Round 2 (commit `b92aeb2`, cookie fix, deployed): ✅ backend `/auth/logout` now emits BOTH a host-only and a `Domain=.douglasdong.com` `cap_session` clear (curl-confirmed live); ✅ authed `/` shows "进入控制台" (nav + hero single primary); ✅ logged-in console data loads — `/tasks` + `/metrics` → 200, dashboard shows real identity (Xeonice + avatar) + capacity (5/5 slots); ✅ logout via AccountMenu → landing `/`, session fully cleared (`/tasks` → 401, anon "GitHub 登录" CTA, no shadow leaked).
