...
template, cron expression, IANA timezone, enabled state, next run time,
overlap policy, misfire policy, and timestamps. A schedule SHALL NOT be modeled
as a `TaskStatus`; each fire SHALL create an ordinary `Task` instance only when
the occurrence is due and admitted for creation.

#### Scenario: Schedule is stored without creating a task immediately
- **WHEN** an account creates a valid schedule for a repo and prompt
- **THEN** the system persists a `TaskSchedule` with the recurrence and task
  template
...
ordinary owner-scoped endpoints.
...
- **AND** the schedule response includes non-secret task template and recurrence
  details
