...
- [x] 5.2 Live verify PASSED (post-deploy, prod, 2026-06-09; tasks `89efda47` + `a18713a2`): backend `ef0b04b` confirmed deployed (`scope` field present) + `METRICS_SAMPLING_ENABLED` on. **P1 no-flicker:** 18 polls × 2 concurrent quiet tasks = 36 reads, `anyNotRunning: false` (zero flicker). **P2 process scope:** every read `scope: process`, codex ≈122 MB vs container ≈0.9–1.1 GiB (~8× overstatement corrected). **Frontend** readout renders "codex CPU 0% · 内存 123 MiB (2%) · 容器 1.1 GiB" (primary + container background by scope). Container-fallback (`scope: container`) path not forced live (hard to trigger deterministically) — covered by unit tests (`resource-sampler-process.test.mjs`, `format-resource.test.ts`). Verify tasks stopped/cleaned up afterward.
