...
- [x] 5.1 Static gates GREEN: api `tsc` 0 / eslint 0 / nest build; `redirect-target.test.mjs` 5/5 + `oauth-callback.test.mjs` 34/34 (incl. deep-link T6d/T6e) + auth suite; web `tsc` 0 / eslint 0 / vitest 60 (incl. `safe-redirect` 3 + `real-auth-session` 3); `@cap/contracts` tsc 0. No `debugger`. (Landing session-aware RENDER + the server-side gate redirect are verified live in 5.2 — the vitest suite is node-env/pure, so the guard + 401→null logic are unit-tested and the SSR redirect is live-checked rather than DOM/SSR-rendered.)
- [ ] 5.2 Live (POST-DEPLOY, pending): **direct-load gate** — logged out, open `/tasks/X` / hard-refresh `/dashboard` directly → server-side 302 to `/login?redirect=…` (NOT a broken shell / 401 page); then login no-deep-link → `/dashboard`; gate-bounced deep-link → returns to `/tasks/X`; unsafe `redirect` → `/dashboard`; logout → landing `/`; authed `/` shows "进入控制台" / anon `/` shows login; visual items render (footer, chips, headline breaks, single primary CTA). NB: the first round of 5.2 (before this fix) FAILED exactly here — the gate did not fire on direct load — which is what surfaced tasks 2.5/2.6. Requires deploy; not committed/deployed per the standing no-push rule.
