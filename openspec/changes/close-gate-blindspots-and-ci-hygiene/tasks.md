...
- [x] 8.9 Prove the job green on a GitHub runner as a non-required check, and record the manual "mark required" branch-protection step as a registered follow-up
  - requirements: ["monorepo-foundation/ci-boots-the-built-application-and-probes-liveness"]
  - surfaces: ["ci"]
  - verify: "workflow-gates"
  > 证据（PR #189）：boot-smoke-stateful 两轮独立 run 均 pass（1m42s / 1m39s，runs 30670136646 与 748c8bc 触发轮）；非 required（分支保护 contexts 未含它）。mark-required 手动步已登记：工件 04 F.1 消费方清单 + proposal Not-in-scope。
...
- [x] 8.10 Prove both lanes green as non-required checks; record the manual required-flip as a registered follow-up GitHub step; if wiring exposes a pre-existing flake, triage three-ways (product defect / stale harness / environment-dependent) and record it — never retry-to-hide
  ...
  - surfaces: ["ci"]
  - verify: "workflow-gates"
  > 证据（PR #189）：web visual regression（2m55s / 3m3s）与 web terminal stories（1m29s / 1m33s）两轮均 pass，均非 required。wiring 暴露的 pre-existing flake = package-suites 里的 boxlite-client 1ms-timeout settlement（非本 change 文件），已按三分法记录为 environment-dependent（工件 04 F.3），留痕后由 F.3 commit push 触发的全新 run 复绿——记录在案，非 retry-to-hide。
...
- [x] 8.12 Compose the PR as the scope-agent-context task 3.4 vehicle: include a docs-only comparison point alongside the backend paths (scripts/ + ci.yml), observe the paths-filter from both sides (run and skip), and record runner-minutes against the 3.4min prediction
  - requirements: ["monorepo-foundation/ci-boots-the-built-application-and-probes-liveness"]
  - surfaces: ["ci"]
  - verify: "workflow-gates"
  > 证据：run 侧 = PR #189（backend 路径，conditioned jobs 全跑且绿：boot-smoke 1m47s、boot-smoke-stateful 1m39s、admission-compat 54s、N-1 1m0s，另 web 两 lane 与 e2e）；skip 侧 = PR #190（docs-only，boot-smoke / admission-compat / N-1 全部 skipping 0s；stateful 与 web lanes 是本 change 新增故不在 main 侧，其 skip 行为由 ci-job-conditions.test.mjs 的同表达式断言推及）。runner-minutes：skip 侧省下的三 job 按 run 侧实测 = 1m47+54s+1m0 ≈ 3.7min，对照 task 3.4 的 3.4min 预测吻合（+9%，未计新增 jobs）。#190 的 package-suites 红 = F.2 已留痕的 aio 套件在 main 代码上的复现（undefined .sent 竞态形态），与两 PR 改动无关，已补记 F.2。
