...
**Reason**: The console is rebuilt for multi-user GitHub OAuth identity. A single shared `AUTH_TOKEN` cannot attribute actions to a person and cannot express a per-identity allowlist; it is replaced by per-user GitHub OAuth sessions gated by a hard GitHub-identity allowlist (see "Allowlist gate is the load-bearing fail-closed security boundary" and "Session establishment and logout" above). The fail-closed refuse-to-start posture is preserved and strengthened: the orchestrator now refuses the OAuth flow when GitHub credentials are missing and denies all logins when the allowlist is empty/unparseable.

**Migration**: Deployments SHALL configure `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET`, the OAuth `redirect_uri`, and a GitHub-`id` allowlist (the prototype's allowlisted account `tanghehui` becomes the seed allowlist entry). The previous shared `AUTH_TOKEN` operator credential is retired for operator login; any tooling that presented `AUTH_TOKEN` as the operator bearer must obtain a GitHub OAuth session instead. The separate runner `TASK_TOKEN` trust domain (sandbox dial-back) is unaffected by this change and remains distinct from operator identity.

#### Scenario: Shared operator token no longer authenticates a human operator

- **WHEN** a client presents the legacy shared `AUTH_TOKEN` as an operator credential against the rebuilt console API
- **THEN** the orchestrator does not establish an operator session from it, because operator authentication now requires a GitHub OAuth session resolving to an allowlisted GitHub identity
