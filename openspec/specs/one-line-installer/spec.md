...
The script SHALL verify prerequisites before mutating the system and SHALL exit with a clear message when they are unmet. The preflight SHALL verify every tool the script invokes (`docker`, `curl`, `bash`) so a host missing a required tool is stopped before fetching the release installer. For macOS default bring-up, the delegated startup path SHALL require BoxLite connection settings before reporting success. For Linux default bring-up, the delegated startup path SHALL verify Docker and the AIO compose path as before.
