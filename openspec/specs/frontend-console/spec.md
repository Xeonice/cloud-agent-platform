...
- **THEN** it uses `queryKeys.sessionHistory(id)` + `sessionHistoryQuery`, with `real.getSessionHistory` validating the payload against the `@cap-console/contracts` `SessionHistoryResponse` schema via Zod `.parse` and a mock fallback selected by the capability flag, mirroring the per-task metrics seam
...
- **THEN** there is no `next` dependency, no `next.config.mjs`/`next-env.d.ts`/Next-shaped `vercel.json`, and `pnpm --filter @cap-console/web build` (a Vite build) succeeds
...
- **WHEN** a `@cap-console/ui` component (e.g. Card) and an `apps/web` surface render on the same page
