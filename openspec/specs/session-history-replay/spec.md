...
- **THEN** it validates the payload against the `@cap-console/contracts` `SessionHistoryResponse` schema via Zod `.parse` before rendering
...
(the channel the frontend already renders as 「推理」), with ZERO change to `@cap-console/contracts` — no new
