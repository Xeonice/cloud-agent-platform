# request-trust-boundary Specification

## Purpose

Fix what the server verifies about an already-authenticated request before it acts
on it. An operation declares the kind of principal it demands — an interactive
operator, never a machine credential that would otherwise inherit the privileges of
the account it happens to belong to — and that kind is checked as a fact of its own,
separately from the account's role. A state change that authenticates by session
cookie SHALL additionally come from an origin the deployment trusts, on the HTTP
path and at the WebSocket handshake alike, because the browser attaches that cookie
no matter which page asked it to. The same boundary governs the code the
orchestrator's own pages may execute: nothing third-party from a floating remote
reference, on the origin that also serves the authenticated API.
