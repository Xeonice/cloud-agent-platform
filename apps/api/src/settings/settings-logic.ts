...
// The slot-ceiling rule moved to `@/task-concurrency/limit` — guardrails needs
// it too, and reaching into settings for it is what made those two directories
// depend on each other. Re-exported so existing settings consumers are unchanged.
export {
  MAX_CONCURRENT_TASKS_MIN,
  MAX_CONCURRENT_TASKS_MAX,
  isValidMaxConcurrentTasks,
  resolveMaxConcurrentTasks,
} from '@/task-concurrency/limit';
