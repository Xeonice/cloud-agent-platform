 ...
 *     approval decision
 *
 * This drives the REAL AioApprovalEnforcer against a stub ApprovalRouter:
...
const enforcerSrc = join(__dirname, 'aio-approval-enforcer.ts');
...
function compile() {
  ...
  const flat = join(outDir, 'aio-approval-enforcer.js');
  if (existsSync(flat)) return flat;
  const hit = findFile(outDir, 'aio-approval-enforcer.js');
  if (hit) return hit;
  throw new Error('compiled aio-approval-enforcer.js not found under ' + outDir);
}
...
async function main() {
  ...
  const { AioApprovalEnforcer, ApprovalDeniedError } = await import(pathToFileURL(enforcerJs).href);

  // --- 1) allow -> action RUNS, result returned -----------------------------
  {
    const router = decisionRouter({ behavior: 'allow', message: 'ok by operator' });
    const enforcer = new AioApprovalEnforcer(router);
    let ran = false;
  ...
  // --- 3) timeout -> no decision -> fail CLOSED; action NEVER runs -----------
  {
    ...
    const enforcer = new AioApprovalEnforcer(hangingRouter, 60); // 60ms timeout
    let ran = false;
    const outcome = await enforcer.enforce({ taskId: TASK_ID, toolName: 'shell', toolInput: {} });
  ...
  // --- 4) router error -> fail CLOSED; action NEVER runs ---------------------
  {
    ...
    const enforcer = new AioApprovalEnforcer(errorRouter);
