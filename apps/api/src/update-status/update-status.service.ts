 ...
 */
export type ReleaseFetcher = (repo: string) => Promise<LatestRelease | null>;
...
export class UpdateStatusService {
  ...
  private async loadLatestRelease(): Promise<CacheEntry> {
    ...
    this.inFlight = (async (): Promise<CacheEntry> => {
      ...
      try {
        const release = await this.fetcher(repo);
...
/**
 * Default {@link ReleaseFetcher}: queries the public GitHub API for the latest
 * published Release of `owner/repo`. Returns `null` for "no published Release"
 * (HTTP 404) or a private/unreachable repo (any non-OK status), so those degrade
 * honestly rather than throwing. A network/transport error rejects and is caught
 * by the service. Uses the global `fetch` (Node 18+); no new dependency.
 */
