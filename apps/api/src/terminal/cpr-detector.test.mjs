 ...
 * import it, drive it with a fake; plain `node`, inline assertions).
 */
...
// ---- compile the REAL bridge to a temp module -------------------------------

// Emit INSIDE apps/api so Node resolution walks up to the repo node_modules for
// `ws` / `@nestjs/common` (a system tmp dir cannot).
const outDir = mkdtempSync(join(apiRoot, '.aio-cpr-test-'));
...
function compileClient() {
  ...
  // tsc follows the transitive (decorator-laden) import graph and preserves the
  // source tree under outDir, so the emitted entry lands under terminal/. Locate
  // it rather than assuming a flat outDir.
  const flat = join(outDir, 'aio-pty-client.js');
  if (existsSync(flat)) return flat;
  const nested = join(outDir, 'terminal', 'aio-pty-client.js');
  if (existsSync(nested)) return nested;
  // Fall back to a recursive search for the compiled entry.
...
async function main() {
  ...
  const clientJs = compileClient();
  const { AioPtyClient } = await import(pathToFileURL(clientJs).href);
