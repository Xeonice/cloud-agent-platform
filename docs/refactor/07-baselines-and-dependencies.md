...
〔**阶段 4 那格的 3,806 是历史基线**：它是**评审时点**的实测值，本表的 ~15× 规模比即按它算出，
故保留不删。该文件此后是长了不是缩了——2026-08-05 活测 **4,131 行**（+325）。
行数今天只作趋势数据：阶段 4 的数字验收目标已于 2026-08-05 被替换为结构判据（见主计划 §4 阶段 4
与工件 08 文末表），本表的规模比不受影响。〕
